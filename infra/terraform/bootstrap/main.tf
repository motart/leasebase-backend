...
# 
# This is a ONE-TIME setup to create core infrastructure in each AWS account.
# Run this BEFORE deploying the application stack.
#
# Usage:
...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "leasebase-terraform-state-${var.environment}-${local.account_id}"
...
resource "aws_dynamodb_table" "terraform_locks" {
  name         = "leasebase-terraform-locks-${var.environment}"
...
resource "aws_ecr_repository" "api" {
  name                 = "leasebase-api-${var.environment}"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:s3:::leasebase-web-${var.environment}-*",
          "arn:aws:s3:::leasebase-web-${var.environment}-*/*"
        ]
      },
      {
